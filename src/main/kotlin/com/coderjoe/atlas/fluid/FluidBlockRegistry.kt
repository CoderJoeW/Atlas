...
class FluidBlockRegistry(plugin: JavaPlugin) : BlockRegistry<FluidBlock>(plugin) {
    ...
    fun registerFluidBlock(fluidBlock: FluidBlock, blockId: String) = register(fluidBlock, blockId)
    ...
    fun getAdjacentFluidBlock(location: Location, face: BlockFace): FluidBlock? = getAdjacentBlock(location, face)
