#!/usr/bin/env python3
"""Generate sci-fi hex-armor textures for Fluid Splitter at 1024x1024.

Concept: Inverse of Fluid Merger — one input splits to many outputs.
Same design language with input/output reversed. Color coding:
  - Input face (front): always RED
  - Output faces (back/side/bottom): GREY when empty, fluid-colored when active
    - water: blue
    - lava: orange
    - experience: green
  - Top face: gauge showing fill level
...
def make_front(fluid="none"):
    """Front face (input): always red connector."""
    return make_face(
...
def make_back(fluid="none"):
    """Back face (output): grey when empty, fluid-colored when active."""
    return make_face(
...
def make_side(fluid="none"):
    """Side face: output connector with horizontal seams."""
    return make_face(
        fluid, FRONT_RING, FRONT_BRIGHT, FRONT_GLOW,
        seam_style="horizontal", is_output=True
    )
...
def make_bottom(fluid="none"):
    """Bottom face: output connector with cross seams."""
    return make_face(
        fluid, FRONT_RING, FRONT_BRIGHT, FRONT_GLOW,
        seam_style="cross", is_output=True
    )
...
def main():
    ...
    for face_name, make_fn in FACES.items():
        for fluid, suffix in FLUID_SUFFIXES.items():
            tex_name = f"fluid_splitter_{face_name}{suffix}"
